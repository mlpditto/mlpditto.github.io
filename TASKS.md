...
## 📅 5 สิงหาคม 2026 (3) — LINE MAN: แถวหัวข้อ "การชำระเงิน" ไม่ตกบรรทัดอีก

*   **วัดผิดรอบแรก ต้องแก้คำรายงาน** — ครั้งแรกวัดว่า "ตกบรรทัดทุกความกว้างตั้งแต่ 1280px" ซึ่ง**ผิด** เพราะเทียบ `top` ของลูกที่สูงไม่เท่ากัน (ปุ่ม GP สูงกว่าชื่อหัวข้อ) **พลาดซ้ำแบบเดียวกับตอนวัดแถบ bulk** → วัดใหม่ด้วยการจับกลุ่มจุดกึ่งกลาง (tolerance 8px) ได้ภาพจริง
*   **อาการจริง:** `.section-header` มี `flex-wrap: wrap` (ใส่ไว้ตอน `1e8e60e` เพื่อให้แถวชิปของหัวข้อ "รายละเอียดการสั่งซื้อ" ตกลงบรรทัดล่างที่ ≤480px) → หัวข้อ "การชำระเงิน" ซึ่ง**ไม่มีชิป** เลยเลือก**ตกบรรทัดทั้งแถว**แทนที่จะปล่อยให้ชื่อย่อด้วย `truncate` + `min-w-0` ที่เตรียมไว้แล้ว; ตกบรรทัดเป็นช่วง ๆ ตั้งแต่ **~560px ลงมา** เวลาปุ่ม GP โชว์ยอดเงิน (สูง 42–46px 2 บรรทัด) เช่น 540/500/481/480/430px สูงพุ่งเป็น 98–107px
*   **แก้ 1 บรรทัด:** `.section-header:not(.has-chips) { flex-wrap: nowrap; }` — หัวข้อที่มีชิปไม่แตะเลย ยัง wrap ได้เหมือนเดิม
*   **ผลหลังแก้:** แถวเดียวทุกความกว้าง 1280→375px ทั้งกรณีมีและไม่มียอดเงิน · ชื่อหัวข้อย่อลงแทน (191px → 64px ที่ 481px) ไม่ถูกซ่อน · ไม่ล้นแนวนอน
*   **ทดสอบ 10 ข้อผ่านหมด** — แถวเดียวครบ 12 ความกว้าง × 2 กรณี (มี/ไม่มียอดเงิน), ชื่อยังโชว์และถูกตัดท้าย, **หัวข้อที่มีชิปยัง wrap ได้และชิปยังตกลงบรรทัดล่างเต็มความกว้างที่ 430px**, ไม่ล้นแนวนอน; รันทุกชุดซ้ำ รวม **361 ข้อ**
*   **ยังไม่ได้ทำในคิวนี้ (user เลือกเฉพาะแถวการชำระเงิน):** หัวโมดัลโต 116→161px ต่ำกว่า 1024px (แถบสรุปตกบรรทัด — **ไม่ใช่ชื่อ "แก้ไขยอดขาย" ตกบรรทัดอย่างที่จดไว้เดิม `<h3>` เป็น 1 บรรทัดเสมอ**) · ป้ายแถบสรุป 4 ตัวยังเข้มเท่ากัน 9px เท่ากัน ขณะที่ตัวเลขไล่ 15/17/15/21px · ชิปเบอร์โทรยุบเหลือไอคอน (กว้าง 134px) · จัดรูปเบอร์โทร `062-594-9253`
*   deploy + verify กับ live แล้ว (ไฟล์เดี่ยว inline ไม่ต้อง bump `?v=`)

## 📅 5 สิงหาคม 2026 (2) — gp-calc: ช่องราคาส่ง + เทียบช่องทางเฉพาะที่กรอกจริง

*   **คำถามที่ค้างมาตลอด ("ปลีก-ส่ง จะใช้ยังไง") โค้ดตอบให้แล้ว ไม่ต้องเดา** — `lineman-mgr.openGpCalc()` แม็ป `m-retail` (RETAIL) → `?store=` พร้อมคอมเมนต์ว่า "store=ราคาปลีก" → **ช่อง "หน้าร้าน" ใน gp-calc คือราคาปลีกอยู่แล้ว** ที่ขาดจริง ๆ คือ **ราคาส่ง (WHOLESALE)**
*   **เพิ่มช่อง "ราคาส่ง"** (`#whole`) + รับผ่าน `?whole=` + `lineman-mgr` ส่ง `m-wholesale` มาให้ → ปิดวงจร master → เครื่องคิดเลขครบทั้งปลีกและส่ง (เดิม `RETAIL`/`WHOLESALE` ใน master เป็นแค่ช่องเก็บค่า ไม่ได้เอาไปคำนวณอะไรเลย)
*   **เปลี่ยนตรรกะเทียบช่องทาง** — `channels()` เดิมรับ 3 ค่าตายตัวและผู้เรียกต้องเช็ก `cost>0 && sell>0 && store>0` ก่อน; ตอนนี้ **นับเฉพาะช่องทางที่กรอกราคามาจริง** แล้วตัดสินเมื่อมี ≥2 ช่องทาง → **กรอกแค่ ปลีก+ส่ง (ไม่มี TELE) ก็เทียบให้แล้ว** ซึ่งเดิมทำไม่ได้; ช่องที่เว้นว่างไม่ไปนั่งอันดับปนกับของจริง
*   **ป้ายช่องเดิมเปลี่ยนเป็น "ราคาขาย (หน้าร้าน/ปลีก)"** ให้เห็นความเชื่อมโยงกับ `RETAIL` ใน master โดยไม่ต้องเดา
*   **ทดสอบ 21 ข้อผ่านหมด (Chromium จริง ผ่าน UI ล้วน)** — `channels()`/`summaryText()` อยู่ใน IIFE เรียกตรงไม่ได้ จึงอ่านผลจากแถบสรุปและกดปุ่มคัดลอกจริง (ดีกว่าเพราะตรงกับทางที่ผู้ใช้เจอ): ครบ 4 ช่องเรียงถูก + ส่วนต่างที่ 1/ที่ 2 ถูก, ราคาส่งสูงกว่าขึ้นเป็นคุ้มสุดได้, ไม่กรอก TELE แล้วไม่มี GP โผล่ปน, ขาดทุนทุกช่องบอกเจ็บน้อยสุด, ช่องทางเดียว/ไม่มีทุน = ไม่ตัดสิน, ข้อความคัดลอกมีบรรทัดราคาส่ง, รับค่าจาก URL ครบ 4 ช่อง, ไม่ล้นจอ 430–360px; รันทุกชุดซ้ำ รวม **351 ข้อ**
*   deploy + verify กับ live แล้ว (ทั้ง `gp-calc.html` และ `lineman-mgr.html` เป็นไฟล์เดี่ยว inline ไม่ต้อง bump `?v=`)

## 📅 5 สิงหาคม 2026 (1) — gp-calc: ปรับ % ขั้นบันไดเองจากหน้าเว็บ (D2)

*   **แท็บ "⏳ ใกล้หมดอายุ" เสร็จอยู่แล้วตั้งแต่ 23 ก.ค.** (`6c70436`) — D2 ที่ค้างคือ 4 ไอเดียเสริมที่ memory จดว่า "ไม่ลงมือจนกว่า user สั่ง"; user เลือกข้อ **UI ปรับ % ขั้นบันได** (อีก 3 ข้อ: ช่องปลีก-ส่ง / QR code / สูตรอายุแบบโค้ง ยังไม่เอา)
*   **เดิม `LADDER` เป็นค่าคงที่ในโค้ด** (เกิน 7 วัน 0% · 4–7 วัน −15% · 2–3 วัน −30% · วันสุดท้าย −50%) แก้ได้เฉพาะคนเปิดไฟล์ → เปิดให้ตั้งเองจากกล่อง **"ปรับขั้นบันไดเอง"** (พับไว้ ไม่แย่งที่ตารางที่ดูทุกวัน) 3 ขั้น × (จำนวนวัน + %)
*   **ป้ายช่วงวันคำนวณจากค่าที่ตั้ง ไม่ใช่ข้อความตายตัว** — `ladTierLabel()` ตั้ง 5/2/1 ก็ได้ `เกิน 5 วัน / 3–5 วัน / 2 วัน / วันสุดท้าย`; ขั้นล่างสุดถ้าไม่ใช่ 1 วันจะเป็น `≤ N วัน` แทนคำว่า "วันสุดท้าย"
*   **ตรวจค่าก่อนใช้** — วันต้องน้อยลงทีละขั้น · ส่วนลด 0–100% · ห้ามติดลบ · ห้ามเว้นว่าง; ค่าผิด = **เตือนแล้วคงตารางเดิมไว้** ไม่ปล่อยให้ราคากระพริบตามค่าที่ยังกรอกไม่เสร็จ
*   **จำค่าไว้ใน `localStorage`** (`gpcalc.ladder.v1`) — ไฟล์นี้เดิมไม่เคยเก็บอะไรเลย; ค่าเสียใน storage = ใช้ค่าเริ่มต้นแทน · โหมดส่วนตัว/quota เต็ม = ข้ามการเซฟ ใช้งานต่อได้ · มีปุ่ม **คืนค่าเริ่มต้น**
*   **🔴 เจอบั๊กจริงระหว่างทดสอบ (ไม่ใช่เทสต์ผิด):** `.ladedit-body` ตั้ง `display:flex` ซึ่ง**ทับการซ่อนเนื้อในของ `<details>` ที่ปิดอยู่** → เปิดกล่องแล้วปิด เนื้อในไม่ยอมหาย (วัดแล้วยัง 36px หลังรอ 2 เฟรม) — **ตระกูลเดียวกับกับดัก `[hidden]` แพ้คลาสที่ตั้ง display ที่เจอมาแล้ว 3 รอบใน cknc** แก้ด้วย `.ladedit:not([open]) .ladedit-body{display:none}` + เพิ่มเทสต์เจาะข้อนี้
*   **ทดสอบ 28 ข้อผ่านหมด** (Chromium จริง โหลดไฟล์จริง) — ค่าเริ่มต้นเหมือนเดิมเป๊ะทั้งป้าย/%/ไฮไลต์ขั้นปัจจุบัน, ปรับ % แล้วราคาขยับตาม (200 −20% = 160) ขั้นอื่นไม่ขยับ, ปรับวันแล้วป้าย+ไฮไลต์คำนวณใหม่, ค่าผิด 4 แบบเตือนและคงตารางเดิม, จำค่าข้ามรีโหลด, ปุ่มคืนค่าเริ่มต้นเขียนทับ storage จริง, เปิด-ปิดกล่องหายจริง, ไม่ล้นแนวนอนที่ 430/390/375/360px; รันทุกชุดซ้ำ รวม **330 ข้อ**
*   deploy + verify กับ live แล้ว — **`gp-calc.html` ไม่ต้อง bump `?v=`** (ไฟล์เดี่ยว JS/CSS inline ทั้งหมด ตามธรรมเนียมในไฟล์ workflow)

## 📅 4 สิงหาคม 2026 (11) — CKNC: แถบ bulk lean UI (D1)

*   **วัดของจริงก่อนเสนอทางเลือก** (memory จดไว้ว่างานนี้ห้ามเดาขอบเขต ต้องถาม user ก่อน) — แถบ bulk ตารางหลักมี 10 ตัวควบคุม (ปุ่ม 8 + select 2 + ช่อง BAR); กลุ่ม BAR กินที่สุด 344px, select งานวางบิล 158px, ประเภทเคส 129px; ตกบรรทัด **2 แถวที่ ≤1280px และ 3 แถวที่ 768px**
*   **user เลือก 3 จาก 4 ข้อ** (ไม่ยุบกลุ่ม BAR — เป็นตัวที่ใช้บ่อยสุด):
    1. **ซ่อนปุ่มที่ใช้ไม่ได้ตอนนั้น** — `applyBulkExcludeVisibility()`: "Exclude" โผล่เฉพาะเมื่อชุดที่เลือกมีใบที่ยังไม่ Exclude · "ยกเลิก Exclude" โผล่เฉพาะเมื่อมีใบที่ Exclude แล้ว · เลือกปนกันโชว์ทั้งคู่ · **ธรรมเนียม Exclude เดิมไม่หาย** (ตามที่ memory เตือน) แค่ไม่โชว์ปุ่มที่กดไปก็ไม่เกิดอะไร
    2. **ยุบปุ่มที่ใช้นาน ๆ เข้าเมนู "เพิ่มเติม ▾"** — แก้เงิน / รวมบิล / ลบรายการ (danger) ใช้ `<details>` + ปิดเองเมื่อคลิกนอกเมนู กดปุ่มในเมนู หรือเปลี่ยนชุดที่เลือก
    3. **จัดกลุ่ม + เส้นคั่น** — จำนวนบิล │ ตั้งค่า (2 select) │ ใส่ BAR │ จัดการบิล │ ล้างการเลือก; ใช้กับแถบใน Card Detail ด้วย (ไม่มีเมนูเพราะไม่มีปุ่มที่ใช้นาน ๆ)
*   **ผลที่วัดได้ (ปุ่มยังครบ 8 ตัว ไม่ได้ตัดทิ้งสักอัน):**

    | จอ | ก่อน | หลัง (กรณีใช้งานจริง) |
    |---|---|---|
    | 1440px | 1 แถว 54px | 1 แถว 54px |
    | 1280px | **2 แถว 98px** | **1 แถว 54px** |
    | 1024px | 2 แถว 98px | 2 แถว 98px |
    | 768px | **3 แถว 136px** | **2 แถว 92px** |

*   **ทดสอบ 19 ข้อผ่านหมด (Chromium จริง ใช้ฟังก์ชัน/listener ตัวจริง)** — ซ่อนตามบริบท 5 เคส (รวมคีย์ที่ไม่มีบิลจริงต้องไม่ระเบิด), เมนูเปิด/ปิดครบ 3 ทาง, ปุ่มยังอยู่ครบ 8 ตัว, ความสูงไม่แย่ลงทุกความกว้าง; รันทุกชุดซ้ำ รวม **302 ข้อ**
*   **กับดักเทสต์ (จดกันลืม):** วัดความสูงแถบที่ 768px ได้ 136/137px สลับกันคนละรอบ = สัญญาณรบกวน sub-pixel ไม่ใช่การถดถอย → ตั้ง tolerance +2px แล้วรันซ้ำ 3 รอบยืนยันเสถียร (อย่าปรับเกณฑ์จนกว่าจะรู้ว่าเป็น noise จริง)
*   deploy + verify กับ live แล้ว (`?v=20260804205239`)

## 📅 4 สิงหาคม 2026 (10) — CKNC: ชิป/แท็บ "รอใบวางบิล" ตัดใบที่ BAR+AR ครบออก (C2)

*   **ตรวจข้ออ้างของ `PLAN-cknc-pending-billing-status-2026-07-17.md` กับโค้ดปัจจุบันก่อน** (แผนอายุ 18 วัน เลขบรรทัดเลื่อนหมดแล้ว) — ข้อหลักยังจริง: `status` คำนวณครั้งเดียวตอน `buildBills` จากการจับคู่ไฟล์ **ไม่มีจุดไหน re-derive จาก BAR/AR เลย** ต่างจาก `billingStage` ที่มี `deriveBillingStage`
*   **🔴 เจอผลข้างเคียงข้ามงานที่แผนยังไม่รู้ — ปุ่มกวาด #10 ที่ลงไปวันเดียวกันทำให้อาการแย่ลง:** ชิปนับด้วย `status === "pending-billing" && !BILLING_WORKFLOW_STAGES.has(stage)` โดยเซ็ตมีแค่ `insurance-review / nhso-pending / pending-review` → บิลที่ BAR+AR ครบเคยถูกซ่อนเพราะ stage เป็น `insurance-review` พอกดกวาดแล้ว stage เป็น `billed` ซึ่ง**ไม่อยู่ในเซ็ต** → **เด้งเข้าชิป "รอใบวางบิล" ตัวเลขเพิ่มขึ้นสวนความคาดหมาย** (โดน 3 ใน 4 สถานะที่ปุ่มกวาด ยกเว้น `general-pending` ที่โผล่อยู่แล้ว); ยังไม่มีใครเจอเพราะข้อมูลชุดปัจจุบันไม่มีใบเข้าเกณฑ์
*   **เลือกทาง ค (user เคาะ) — แก้ที่ชั้นแสดงผลอย่างเดียว:** `isPendingBillingOpen(bill)` ตัด 2 กลุ่มออกจากชิป/แท็บ — (1) บิลที่จัดเข้าหมวดงานวางบิลเฉพาะแล้ว (de-dup เดิม) (2) **บิลที่มีเลข BAR+AR ครบ** = วางบิลเสร็จจริงแม้ status ยังเป็น `pending-billing`; ใช้แพตเทิร์นเดียวกับ `isReconciledClicknicOnly` ที่มีอยู่แล้วในไฟล์
*   **ไม่เลือกทาง ก (re-derive `status`)** เพราะกฎ validation + ตัวนับแท็บ/export ผูกกับ `status` หลายที่ (`MISSING_BILLED_AMOUNT` จะเด้งเตือนใหม่ทั้งกอง); **ไม่เลือกทาง ข** (ปลดล็อก manual อัตโนมัติ) เพราะจะเขียนทับค่าที่ผู้ใช้เลือกเองเงียบ ๆ — #10 ทำหน้าที่นี้แบบกดเองอยู่แล้ว
*   **ขอบเขตที่ตั้งใจไม่แตะ:** `metrics.mlpNoBilling` (ตัวนับดิบ ใช้ในการ์ด/export/PDF 3 ที่) ยังนับ `status === "pending-billing"` ตรง ๆ เหมือนเดิม — ความไม่ตรงกันระหว่างตัวนับดิบกับชิปมีมาก่อนแล้ว ไม่ได้เกิดจากรอบนี้
*   **ทดสอบ 19 ข้อผ่านหมด** — เคสจากแผนจริง (order `0202607031149461`) ไม่นับทั้งก่อน/หลังกวาด, PAID ไม่นับ, มีแต่ BAR หรือมีแต่ AR ยังนับ, เลขเป็นเว้นวรรคยังนับ, de-dup เดิมทั้ง 3 สถานะไม่พัง, สถานะอื่นไม่เกี่ยว, กองรวม 6 ใบนับได้ 2 ใบที่ถูกต้อง; รันทุกชุดซ้ำ รวม **283 ข้อ**
*   **ยังค้าง (ต้อง login):** ขั้นที่ 1 ของแผน — ยิง console แยก H1/H2/H3 ว่าทำไม `status` ถึงเป็น `pending-billing` ทั้งที่มีเลขครบ (กรอกมือล้วน / จับคู่ ORW-INV พลาด / แก้ ORW แล้ว matching ไม่ rerun) **ถ้าเป็น H2/H3 = บั๊กการจับคู่ ต้องแก้ที่ต้นทางเพิ่ม** รอบนี้แก้แค่ให้ตัวเลขบนจอไม่หลอกตา
*   deploy + verify กับ live แล้ว (`?v=20260804203444`)

## 📅 4 สิงหาคม 2026 (9) — CKNC: รายงานยอดเงินเข้า เพิ่มสรุปรายเดือน (C1 ต่อยอด)

*   **ตรวจพบก่อนลงมือว่า C1 ทำเสร็จอยู่แล้ว** — รายงาน "สรุปยอดเงินเข้าตามวันรับเงิน" (`renderPayoutReport` + `payoutModal` + ปุ่ม `openPayoutBtn` ที่การ์ด "ยอดเงินเข้า") ขึ้น live ตั้งแต่ **16 ก.ค. 2026 commit `293dcce`** · memory `fkb-roadmap-phases` และ `cknc-cost-model` ตกข่าว (ยังเขียนว่า "ยังไม่มีรายงาน") — แก้ทั้งสองไฟล์แล้ว
*   **ทดสอบของเดิมก่อนแตะ 12 ข้อผ่าน** (Chromium จริง) ยืนยันเกณฑ์ถูก: นับเฉพาะ PAID + มีวันรับเงิน + ไม่ Exclude · จัดกลุ่ม วัน × BAR · บิลไม่มี BAR ขึ้นกลุ่ม `(ไม่มี BAR)` ไม่หายเงียบ · ไม่มีข้อมูลขึ้นข้อความแนะนำ
*   **ของใหม่ที่เพิ่ม: ตารางสรุปรายเดือน** เหนือรายวัน — เดือนที่ได้รับเงิน / จำนวนบิล / **จำนวนใบวางบิล (นับ BAR ไม่ซ้ำ)** / ยอดขาย / กำไร เรียงใหม่→เก่า พร้อมหัวข้อคั่น 2 ส่วน (`สรุปรายเดือน` / `รายวัน × ใบวางบิล`)
*   **จุดที่คิดแยกจากรายวัน:** คิดจากตัวบิลตรง ๆ ไม่ใช่จาก `rows` ที่ยุบตาม วัน×BAR ไปแล้ว — **ใบวางบิลใบเดียวที่ได้เงินหลายวันต้องนับเป็น 1 ใบต่อเดือน** (เทสต์เจาะข้อนี้: BAR-364 ได้เงิน 2 วัน → เดือนนั้นนับ 4 ใบไม่ใช่ 5)
*   **ใช้ `monthChipLabel()` ตัวเดิม** ไม่เขียน formatter ใหม่ — รองรับสลับ พ.ศ./ค.ศ. ตาม `yearEra` อยู่แล้วตามธรรมเนียมโปรเจกต์
*   **ทดสอบรวม 19 ข้อของรายงานนี้ผ่านหมด** (เพิ่ม 7 ข้อสำหรับสรุปรายเดือน + ยืนยันว่ารายวันของเดิมไม่พัง: BAR เดียวกันคนละวันยังแยกแถว, ในวันเดียวกันยังเรียงตามชื่อ BAR, ยอดรวมทุกเดือน = ยอดรวมด้านบน); รันทุกชุดซ้ำ รวม **264 ข้อ**
*   **ยังไม่ได้ทำ (เสนอไว้ user เลือกแค่สรุปรายเดือน):** คอลัมน์ยอดใบวางบิล (`billedAmount`) คู่ยอดขายเพื่อเทียบ "เรียกเก็บ vs ได้จริง" · ปุ่ม export CSV · ผูกกับตัวกรองวันที่บนจอ
*   deploy + verify กับ live แล้ว (`?v=20260804200652`)
