...
## 📅 4 สิงหาคม 2026 (8) — CKNC: ปุ่มกวาด "BAR+AR ครบแต่ยังไม่วางบิลแล้ว" (#10 ตัวสุดท้ายในคิว)

*   **ส่วนที่ 3 ในโมดัล WARN** ต่อจาก "น่าจะเป็นบิลเดียวกัน" และ "สปสช ต้นทุน MLP ไม่ใช่ 0" — ตาราง + ปุ่ม "ปรับเป็นวางบิลแล้ว" + ปุ่มดินสอรายบิล (แพตเทิร์นเดียวกับส่วน สปสช)
*   **เกณฑ์แคบตามที่ออกแบบ:** มี BAR **และ** AR ครบ **และ** `billingStage` อยู่ใน 4 สถานะ "รอ" (`pending-review` / `insurance-review` / `nhso-pending` / `general-pending`) — ไม่แตะ `paid` (ปลายทาง) / `billed` (ถูกอยู่แล้ว) / `cancelled` (ตั้งใจ) / `billing-only` / `no-mlp` (ผลการจับคู่ ไม่ใช่สถานะงาน); ยึด `activeBills()` ไม่ผูกตัวกรองวันที่ (Exclude ตกไปเอง)
*   **เขียนบล็อกที่ 2–3 ใหม่ทั้งหมด** — เอกสารเตือนเองว่าเขียนจากโค้ดเครื่องร้านซึ่งเก่ากว่า live และ **จุดที่ 2–3 เจอ 0 นัด**; ของจริงตอนนี้ `warnTotalCount()` = `mergeSuggestTotal() + nhsoCostIssues().length` และหัวข้อใช้ตัวแปร `suggTotal` (ขยับอีกจาก #7 ที่เพิ่งลง) → เปลี่ยนหัวโมดัลมาใช้ `warnTotalCount()` ตัวเดียวกับที่อื่น จะได้ไม่มีสูตรนับสองชุด
*   **ตั้ง `billingStageSource = "auto-billing"` ไม่ใช่ `manual`** — คืนบิลให้ระบบดูแลต่อ ถ้าลบ BAR/AR ทีหลังสถานะจะถอยกลับเอง ไม่ค้างเป็น "วางบิลแล้ว" ทั้งที่ไม่มีเลข
*   **มี `confirm()` ก่อนกวาด** (ต่างจากปุ่ม Set MLP cost 0 ที่ทำเลย) เพราะปุ่มนี้**ขยับตัวเลขรายได้** — `countsInRevenue` นับ `billed` + มี BAR เป็นรายได้จริง; ข้อความบอกจำนวนใบ จำนวนใบที่เป็น "แก้มือ" และเตือนผลข้างเคียงตรง ๆ
*   **ตัดโค้ดซ้ำที่เกือบเขียนเกิน:** เดิมจะสั่งปิด/รีเฟรชโมดัลเองหลังกวาด แต่ตรวจแล้วพบว่า `applyBulkOverride` → `renderTable()` → `renderMergeSuggestions()` ปิด/รีเฟรช popup ให้อยู่แล้ว (กติกาเดียวกับปุ่ม Set MLP cost 0) — เอาออก ไม่ render สองรอบ
*   **ต่างจากแพตช์เล็กน้อย:** คอลัมน์ ออเดอร์/ORW ใช้ `orderOrwCellHtml()` (แพตช์เขียน `bill.orderId || bill.orw`) → ได้ปุ่มคัดลอกจาก #9 มาด้วยและตรงกับตาราง สปสช ที่อยู่ข้างบน
*   **ทดสอบ 24 ข้อของงานนี้ผ่านหมด** — logic 10 (4 สถานะเข้าเกณฑ์ครบ, 7 สถานะต้องห้ามไม่จับสักใบ, ต้องมีเลขครบทั้งสองตัว/เว้นวรรคไม่นับ, Exclude ตกเอง, source ไม่ใช่เงื่อนไข, `warnTotalCount` นับรวม) + DOM 14 (Chromium จริง: หัวข้อ+จำนวน, หัวโมดัลนับรวม, PAID ไม่โผล่, ป้าย "แก้มือ" ถูกใบ, ข้อความ confirm ครบ 3 ประเด็น, สั่งแก้เฉพาะคีย์ที่เข้าเกณฑ์ ด้วยค่า `billed`+`auto-billing`, ปุ่มดินสอเปิด drawer ถูกใบ); รันทุกชุดซ้ำ รวม 245 ข้อ
*   **⚠️ ยังเป็นของเงียบบนข้อมูลชุดปัจจุบัน** — จำลองสัดส่วนจริง (billed 139 · paid 100 · no-mlp 21 · billing-only 7) ได้ 0 ใบเข้าเกณฑ์ ส่วนที่ 3 จะไม่โผล่จนกว่าจะมีใบค้างจริง
*   **⚠️ คำถามที่ยังไม่มีคำตอบ (เอกสารตั้งไว้เอง):** ถ้าใบค้างส่วนใหญ่เป็น `manual` ทั้งที่ไม่เคยตั้งใจกด = แถบ chip งานวางบิลล็อกสถานะถาวร **ต้องไปแก้ที่ต้นทาง ไม่ใช่มากวาดทีหลัง** — ก่อนกดปุ่มจริงให้ยิง console ตามตารางตัดสินใจในเอกสารแพตช์ก่อน
*   deploy + verify กับ live แล้ว (`?v=20260804100552`)

## 📅 4 สิงหาคม 2026 (7) — CKNC: รวมบิลแบบ bulk เลิกรวมได้ถาวร (ปลดล็อกงานรวมคู่ ORW+AR 129 คู่)

*   **ด่านสุดท้ายที่ค้างจาก #3** — #3 ทำให้การรวม*ทีละคู่*เลิกรวมได้ข้ามวัน แต่ `bulkMergeGroups` (ปุ่ม "รวมคู่ ORW ทั้งหมด" / "รวมที่แน่นอน") ยังเก็บสำเนาไว้ใน closure ของ toast → รวม 129 คู่แล้วปิดหน้า = เลิกรวมไม่ได้เลย
*   **แก้แบบเดียวกับ #3 แต่เป็นระดับชุด:** ทุกกลุ่มที่เกิดจากการกดครั้งเดียวกันได้ `batchId` ร่วมกัน + เก็บ `members` ลงในตัวกลุ่ม (ตัด `validationIssues` ทิ้ง — สำคัญมากตอน 129 กลุ่ม) + แถวประวัติผูก `mergeBatchId` → ปุ่ม **"เลิกรวมทั้งชุด (N กลุ่ม)"**
*   **`unmergeBatch()` ทำ audit แถวเดียว + rebuild ครั้งเดียว** — วนเรียก `unmergeGroup` ทีละกลุ่มจะได้ประวัติ 129 แถวและ rebuild 129 รอบ; ถามยืนยันก่อนเพราะคืนหลายร้อยบิลรวดเดียว (ต่างจากรายกลุ่มที่คืนแค่ 2 ใบ)
*   **toast ใช้เส้นทางเดียวกับปุ่มในประวัติแล้ว** (`unmergeBatch(batchId)`) — ไม่มีสำเนาซ้อนใน closure อีก; แยก `restoreMergeGroupBills()` ให้ทั้งรายกลุ่มและทั้งชุดใช้ร่วม
*   **กันไว้ตอน 129 กลุ่ม:** `groupId` เดิมเป็น `merge-${Date.now()}-${สุ่ม 6 หลัก}` — รวมทีเดียวทุกกลุ่มเกิดใน ms เดียวกัน มีโอกาสชนกันเงียบ ๆ (~0.05% ที่ 129 กลุ่ม) เติมลำดับในชุดเข้าไปด้วย → เทสต์ยืนยัน id ไม่ซ้ำสักตัวใน 129 กลุ่ม
*   **ชุดเก่า/ชุดปนกัน:** ถ้ามีกลุ่มไหนในชุดไม่มีสำเนาและอยู่โหมด snapshot → เตือนแล้ว**ไม่ทำทั้งชุด ไม่ลบกลุ่มไหนเลย** (กันบิลหาย); โหมดมีไฟล์ต้นทางทำต่อได้ปกติ
*   **ทดสอบ 39 ข้อของงานนี้ + 15 ข้อ DOM (รวมทุกชุด 221 ข้อ) ผ่านหมด** — จำลองทิ้ง closure ทั้งหมดแล้วเลิกรวมทั้งชุดยังได้ บิลกลับมาครบและอยู่ตำแหน่งเดิม (ใส่บิลเดี่ยวคั่นกลางเพื่อพิสูจน์), ประวัติแถวเดียวไม่ใช่ 3 แถว, กดซ้ำ/id มั่ว = false, เลิกรวมรายกลุ่มยังทำงานหลัง refactor, ปุ่มในประวัติป้ายบอกจำนวนกลุ่ม + ถามยืนยัน + เรียกด้วย batchId ที่ถูก
*   **ขนาด session:** วัดกับบิลตัวอย่างเล็ก 129 คู่ได้ +23 KB — **บิลจริงมีรายการยา ตัวเลขจริงสูงกว่านี้มาก ต้องวัดบน live ก่อนกดรวมจริง** ที่ console: `JSON.stringify({bills: state.bills, billMergeGroups: state.billMergeGroups}).length` วัดก่อน/หลังรวม 10 คู่ แล้วคูณ 13
*   deploy + verify กับ live แล้ว (`?v=20260804094511`) — โค้ด closure เดิม (`originals.forEach`) เหลือ 0 นัด

## 📅 4 สิงหาคม 2026 (6) — CKNC: ปุ่มคัดลอกท้าย ORW / เลขที่ออเดอร์ ในตาราง WARN และ AR2BAR

*   **ปุ่มแยกต่อบรรทัด** ในคอลัมน์ ออเดอร์/ORW (เลข 2 บรรทัดในเซลล์เดียว) — บางงานใช้ ORW บางงานใช้เลขที่ออเดอร์ จึงไม่ทำปุ่มเดียวรวม; ทั้งตาราง WARN (สปสช ต้นทุน MLP ไม่ใช่ 0) และตาราง AR2BAR ใช้ `orderOrwCellHtml` ตัวเดียวกัน → ได้ปุ่มพร้อมกันทั้งสองที่
*   **ปัญหาที่ต้องแก้พร้อมกัน:** handler `[data-copy-text]` มีอยู่ **5 ที่** ในไฟล์แบบก๊อปวางซ้ำ ๆ แต่ `mergeWarnBody` กับ `barPickerBody` **ไม่มี** → ถ้าเติมแต่ปุ่มจะได้ปุ่มที่กดแล้วเงียบ; แยกเป็น `attachCopyDelegate(el)` แล้วเรียกเฉพาะ 2 กล่องใหม่ — **ของเดิม 5 ที่ไม่แตะ** (การยุบเป็นงาน refactor คนละก้อน ต้องทดสอบปุ่มคัดลอกครบทั้ง 5 ที่)
*   **ไม่เพิ่ม CSS** — `.copy-ref-btn` + `<i class="fa-regular fa-copy">` เป็นแพตเทิร์นมาตรฐานของไฟล์อยู่แล้ว
*   **ต่อจาก #6:** ใช้เว้นวรรคใน tooltip เหมือนกัน (`คัดลอก ORW` / `คัดลอก เลขที่ออเดอร์`)
*   **ทดสอบ 14 ข้อผ่านหมด (Chromium จริง):** จำนวนปุ่มตามข้อมูลที่มี (2/1/1/0 ปุ่ม, ไม่มีอะไรเลย = `-`), tooltip แยกตามบรรทัด, กดแล้วได้ค่าตรงบรรทัด, ORW หลายเลขคั่นจุลภาคคัดลอกทั้งก้อนตรงกับที่แสดง, ไอคอนเปลี่ยนเป็น ✓
*   **regression ที่ตั้งใจวัด (เพราะเพิ่ม listener ตัวที่สองบน element เดิม):** ผูก handler เดิมของโมดัล WARN ตัวจริงเข้าไปด้วย แล้วกดปุ่มคัดลอก → **handler เดิมไม่ถูกเรียกสักตัว** (ดินสอ/Set MLP cost 0/เทียบ/รวม/เปลี่ยนหน้า) และ checkbox ในแถวไม่ถูกติ๊กทั้งตาราง WARN และ AR2BAR; รันชุดเดิมทั้งหมดซ้ำ รวม 178 ข้อ
*   **กับดักเทสต์ (จดกันลืม ซ้ำรอยที่เคยจด):** `className` ถูกตั้งใน `.then()` ของ clipboard → ต้อง `await Promise.resolve()` ก่อนอ่าน ไม่งั้นได้ค่าเก่า
*   deploy + verify กับ live แล้ว (`?v=20260804092941`)

## 📅 4 สิงหาคม 2026 (5) — CKNC: แบ่งหน้ารายการ "น่าจะเป็นบิลเดียวกัน" หน้าละ 10 คู่

*   **เดิมเห็นแค่ 8 คู่จาก 114** — `computeMergeSuggestions` ตัด `.slice(0, 8)` ตั้งแต่ชั้นคำนวณ; ย้ายการตัดจำนวนไปเป็นเรื่องของชั้นแสดงผลแทน (แบ่งหน้า 10 คู่/หน้า ใช้คลาส `.card-detail-pager` เดิม ไม่ต้องเพิ่ม CSS)
*   **`state.mergeSuggestionsTotal` ตั้งไว้ก่อนตัดอยู่แล้ว** → `mergeSuggestTotal()` / `warnTotalCount()` / หัวโมดัลไม่เปลี่ยนค่า ไม่ต้องแก้ตามที่อื่น (ยืนยันด้วยเทสต์ว่าหัวโมดัลยังเป็นจำนวนเต็ม)
*   **🔴 จุดที่พังง่ายสุดและกันไว้แล้ว:** ปุ่ม เทียบ/รวม ส่ง index ให้ handler ที่หยิบด้วย `state.mergeSuggestions[index]` — ถ้าแบ่งหน้าแบบ `pageItems.map((item, index))` ตรง ๆ index จะกลายเป็น 0-9 ของหน้านั้น **หน้า 2 กด "รวม" แถวแรกจะไปรวมคู่ที่ 1 = รวมบิลผิดคู่แบบเงียบ ๆ (เสียข้อมูลจริง)**; ใช้ `const index = pairStart + offset` ทุกจุด และมีเทสต์เจาะข้อนี้โดยเฉพาะ
*   **หน้าไม่เด้งกลับ 1 หลังรวม/ซ่อนคู่** — รีเซ็ตที่ `openMergeWarnModal` ที่เดียว ที่เหลือให้ clamp ในตัว render จัดการ (คู่ที่เหลือเลื่อนขึ้นมา ผู้ใช้อยู่หน้าเดิมต่อได้); ตรวจแล้วว่าไม่มี `mergeSuggestPage = 1` หลุดไปอยู่ใน render
*   **ทดสอบ 22 ข้อผ่านหมด (Chromium จริง, ใช้ `renderMergeWarnBody` + listener ตัวจริง):** หน้า 1 = 10 แถว/`หน้า 1/12`/`แสดง 1–10 จาก 114`, หัวโมดัลยัง 114, หน้า 2 index เป็น 10–19 ไม่ใช่ 0–9, **กดเทียบแถวแรกหน้า 2 ได้คู่ที่ 11**, หน้าสุดท้าย 4 แถว ปุ่มถัดไปจาง, ค้างหน้า 12 แล้วคู่ลดเหลือ 12 → clamp มา 2/2 ไม่ค้างหน้าว่าง, 6 คู่ = ไม่มีแถบแบ่งหน้าเลย; รันชุดเดิมทั้งหมดซ้ำ รวม 164 ข้อ
*   **regression:** ปุ่ม "รวมคู่ ORW ทั้งหมด" / "รวมที่แน่นอน" ใช้ `orwComplementGroups()` / `certainMergeGroups()` ที่คำนวณจาก `state.bills` เอง ไม่แตะ `state.mergeSuggestions` → การแบ่งหน้าไม่กระทบ (ตรวจโครงสร้างแล้ว ยังต้องกดยืนยันบน live สักครั้ง); ส่วน สปสช ไม่ถูกแบ่งหน้า แสดงครบเหมือนเดิม
*   deploy + verify กับ live แล้ว (`?v=20260804092201`) — `.slice(0, 8)` ตัวที่ต้องเอาออกเหลือ 0 นัด อีก 5 ที่ที่ไม่เกี่ยวยังอยู่ครบ

## 📅 4 สิงหาคม 2026 (4) — CKNC: ตารางเทียบคู่บิล เพิ่มแถว Ref-ID + INV ให้ครบชุดเลขระบุตัว

*   **เลขระบุตัวเรียงติดกันครบชุด** ในป๊อปอัพ "น่าจะเป็นบิลเดียวกัน": ผู้รับบริการ → เลขที่ออเดอร์ → **Ref-ID** → ORW → **INV** → สถานะ (เดิมมีแค่ออเดอร์กับ ORW) — ชุดนี้คือตัวที่ใช้ตัดสินว่าใบเดียวกันหรือไม่
*   **Ref-ID กับ INV เป็นแถว "มีค่อยโชว์"** ซ่อนเมื่อว่างทั้งสองใบ — `refId` ถูกตั้งเป็น `""` ตอนสร้างบิลเสมอ ไม่มีทาง import เข้ามา (เติมได้แค่พิมพ์เองหรือให้ paste-analyze แกะให้) ถ้าโชว์ตายตัวจะได้แถว `-` เทียบ `-` เพิ่มมาในเกือบทุกคู่ของตารางที่ยาว 15 แถวอยู่แล้ว; มีข้างเดียวก็ยังโชว์ (ยังบอกอะไรได้)
*   **ปุ่มคัดลอกครบ 4 แถว** พร้อม tooltip ตามชื่อแถวจริง (เดิม hardcode "คัดลอกเลขที่ออเดอร์" ทุกปุ่ม) — ใช้ delegated handler `[data-copy-text]` ของ `suggestPairBody` ที่มีอยู่แล้ว ไม่ต้องผูก listener เพิ่ม
*   **ต่างจากแพตช์ 1 ตัวอักษร:** แพตช์เขียน `คัดลอก${label}` ติดกันได้ `คัดลอกRef-ID` → เติมเว้นวรรคให้ตรงกับธรรมเนียมเดิมในไฟล์นี้ (`คัดลอก Ref ID` ที่ `cknc.js:3500`)
*   **ทดสอบ 11 ข้อผ่านหมด (Chromium จริง, render ด้วย `openSuggestPairModal` ตัวจริง + listener ตัวจริง):** ไม่มี Ref-ID ทั้งคู่แถวไม่โผล่, มีข้างเดียวแทรกถูกตำแหน่ง, INV ว่างทั้งคู่แถวหายแต่แถวอื่นอยู่ครบ, ช่องว่างขึ้น `-`, ค่าต่างกันได้พื้นเหลือง `pair-diff`, tooltip/ค่าที่ก๊อปถูกรายแถว, กดปุ่มแถว ORW ได้ค่า ORW; รันชุดเดิมทั้งหมดซ้ำ รวม 142 ข้อ
*   deploy + verify กับ live แล้ว (`?v=20260804091146`) — `คัดลอกเลขที่ออเดอร์` แบบ hardcode เหลือ 3 ที่ (การ์ด/ตารางอื่น ไม่ใช่ตารางเทียบ) ตามที่ควรเป็น

## 📅 4 สิงหาคม 2026 (3) — CKNC: คอลัมน์ "ตรวจสอบ" ยุบซ้ำ + เรียงความรุนแรง + โชว์ตัวเลขส่วนต่าง

*   **ยุบ issue ที่ทุกแถวติดเหมือนกัน** ขึ้นหัวการ์ดเป็น chip `ทุกแถวติดเหมือนกัน: NBR ไม่มีเลข BAR ×16` — เกณฑ์คือ **code + ข้อความเต็มตรงกัน** ไม่ใช่ดูแค่ code ตัวที่พกข้อมูลรายบิล (DMM วันที่ / BCM ส่วนต่าง) จึงไม่ถูกยุบเองโดยไม่ต้องมีลิสต์ยกเว้นให้ลืมอัปเดต; ขอบเขต = เฉพาะแถวในหน้าที่แสดง ตรงกับ chip `ค่าเดียวกันทั้งการ์ด:` เดิม
*   **เรียงตามความรุนแรง** danger → warn → info (ที่เท่ากันคงลำดับ rule เดิม เพราะ `Array.sort` เสถียร) + **ตัดเหลือ 2 chip** ที่เหลือเป็นปุ่ม `+N` กดกาง (chip ส่วนเกินอยู่ใน DOM แล้ว ซ่อนด้วย CSS → กดแล้วกางได้ไม่ต้อง re-render)
*   **ตัวเลขส่วนต่างบน chip** — `pushIssue` รับ `diff` เพิ่ม (ใส่ field เฉพาะตอนเป็นตัวเลขจริง กัน `undefined` ลง Firestore) ใช้กับ COS/BEM/BCM/ECM โดยยึด "ยอดวางบิล" เป็นตัวตั้งเสมอทั้ง 3 กติกา → chip อ่านได้ `BCM ยอดไม่ตรง MLP +218.94` โดยไม่ต้อง hover; ใช้ − (U+2212) ไม่ใช่ hyphen
*   **🔴 กับดักที่เกือบทำ commit เสียทั้งไฟล์:** ตัวคั่นใน `issueSignature` ควรเป็น escape `\u0000` แต่ตอนแก้กลายเป็น **อักขระ NUL จริง** ฝังในไฟล์ → git มองไฟล์เป็น binary เลย**ข้ามการ normalize CRLF ทั้งไฟล์** ทำให้ `git diff` ขึ้น **10,768 บรรทัด** ทั้งที่แก้จริงไม่กี่จุด; แก้เป็น escape แล้ว diff กลับมา 89/13 — **ถ้าไม่ทันเห็นจะ commit ทับทั้งไฟล์และ blame ใช้ไม่ได้** (ต้นเหตุ: backslash ถูกกินไปชั้นหนึ่งตอนส่งคำสั่งผ่าน shell — เลี่ยงด้วย `String.fromCharCode(92)`)
*   **ทดสอบ 44 ข้อของแพตช์นี้ผ่านหมด** — logic 34 (ฟังก์ชันจริง: diff เก็บ/ไม่เก็บ, NaN/Infinity/0, เครื่องหมาย U+2212, เกณฑ์ 0.005, เรียงเสถียร, ยุบ 16 แถว, DMM วันที่ต่างกันห้ามยุบ, การ์ดปน วางบิลแล้ว/PAID ห้ามยุบ, แถวเดียวไม่ยุบ, การ์ดสะอาดไม่โผล่แถบว่าง) + DOM 10 (Chromium จริง ใช้ listener ตัวจริง: chip อยู่ครบ 4 ใน DOM โชว์ 2 ซ่อน 2, เรียงแดงมาก่อน, กด +N กางครบ ปุ่มหาย, ใส่ `show-all`)
*   **regression:** รายงาน/export (`validationReportRows`, `issueText`) ใช้ `bill.validationIssues` เต็มโดยตรง ไม่ผ่านตัวยุบ → ยังเห็น issue ครบทุกตัวรายบิล; รันชุดเทสต์เดิมทั้งหมดซ้ำผ่าน (รวม 131 ข้อ)
*   **กับดักเทสต์ (จดกันลืม):** `vm.runInContext("let x = ...")` สร้าง binding ใน script scope เซ็ตทับจากฝั่ง node ไม่ได้ (ต้องใช้ `var`) · ใน `page.evaluate` ต้องใช้ **indirect eval** ไม่งั้นฟังก์ชันตกอยู่แค่ scope ของ callback และ `const` ใน indirect eval ไม่กลายเป็น global ต้องแปลงเป็น `window.X` · chip อยู่ใน flex container → `inline-flex` ถูก blockify เป็น `flex` ให้วัด "ซ่อน/ไม่ซ่อน" แทนชื่อค่า
*   deploy + verify กับ live แล้ว (`?v=20260804090116`) — เช็กด้วยว่าไม่มี NUL หลุดขึ้น live

## 📅 4 สิงหาคม 2026 (2) — CKNC: แถวปุ่มโมดัล paste analyze ไม่ห่อบรรทัด + ปุ่มบอกจำนวนช่อง

*   **อาการ:** ปุ่ม `ยกเลิก` / `นำไปใช้กับบิลนี้` ห่อเป็น "ยก/เลิก" — `.modal-actions` เป็น flex `space-between` แต่ไม่มีใครถูกล็อกไม่ให้หด พอข้อความซ้าย (`บิล: ${bill.patient}` ที่บางใบปนข้อความดิบมาทั้งบรรทัด) ยาว กล่องปุ่มเลยโดนบีบ
*   **แก้ตามที่ตกลง:** ย้ายชื่อบิลไปใต้หัวข้อโมดัล (`#pasteAnalyzeTarget` + `.modal-sub` มีที่ให้ยาว, เลขที่ออเดอร์นำหน้าชื่อ) แถวล่างเหลือปุ่มล้วน + ปุ่มหลักบอกจำนวนช่อง (`นำไปใช้ N ช่อง` / ว่าง = `เลือกช่องที่จะใช้` ตอบในตัวว่าทำไมกดไม่ได้); รีเซ็ตตอนเปิดโมดัลและตอนข้อความว่างใช้ `updatePasteApplyState()` ตัวเดียวกัน ไม่ตั้ง `disabled` มือ
*   **วัดของจริงด้วย Chromium เทียบ `git show HEAD:` ก่อน/หลัง** — ก่อนแก้: ข้อความยาวทำปุ่มสูง 40→44px และถ้ายาวมากปุ่มสูง **128px** + ปุ่ม "นำไปใช้" ถูกบีบเหลือกว้าง **54px**; หลังแก้: ค้าง 40px กว้าง 144px ทั้งจอกว้างและ 800px
*   **เจอเพิ่มระหว่างทดสอบ — แก้เกินแพตช์ 1 บรรทัด:** แพตช์อ้างว่าข้อความซ้ายจะ "ตัดท้ายด้วย …" แต่จริง ๆ **ไม่ตัด** กลับดันแถวกว้างออกไป 2907px ล้นกรอบโมดัล 780px เพราะ `.modal-actions` เป็น **grid item** ของ `.modal-card` ซึ่ง `min-width` ปริยาย = `auto` (หดต่ำกว่า min-content ไม่ได้) → เติม `min-width: 0` ให้ `.modal-actions` แล้ว `sumClipped` เป็น true ตัดท้ายจริง **กับดักเดียวกับตอนปุ่ม AR2BAR ล้นช่อง AR (`758009d`)**
*   **regression โมดัลอีก 10 ตัวที่ใช้ `.modal-actions` ร่วมกัน** (screenshot/clipboard/mergeResult/importMode/importResult/dupWarn/suggestPair/dismissReason/barPicker/bulkMoney): ตัวเลข `rowH`/`maxBtnH`/ล้นขวา เท่ากันเป๊ะกับก่อนแก้ทุกตัว
*   **ทดสอบรวม 87 ข้อผ่านหมด** — ของแพตช์นี้ logic 12 ข้อ (ฟังก์ชันจริง: เลขนำหน้าชื่อ, fallback ORW, ชื่อว่าง, นับช่อง 0/1/3/1,234, เอาติ๊กออกหมดไม่ค้างเลขเดิม) + layout Chromium เทียบ HEAD; และรันชุดเดิมซ้ำครบ (bar-logic 20, css-cascade 14, unmerge 30, unmerge-dom 11)
*   deploy + verify กับ live แล้ว (`?v=20260804083750`) — ยืนยันว่าข้อความเก่า `` textContent = `บิล:` `` เหลือ 0 นัดบน live

## 📅 4 สิงหาคม 2026 (1) — CKNC: เลิกรวมบิลได้ถาวร ไม่ผูกกับ toast

*   **ปัญหา:** `unmergeGroup` รับสำเนาบิลต้นฉบับจาก **closure ของ toast** → toast หาย/รีเฟรช/เปิดวันใหม่ = **เลิกรวมไม่ได้อีกเลย**; แค่ลบกลุ่มแล้ว rebuild ไม่พอเพราะโหมด snapshot ไม่สร้างบิลใหม่จาก source และ session เก็บ `payload.bills` ที่เป็นบิล**หลังรวม**ไปแล้ว
*   **แก้:** ย้ายสำเนาสมาชิกไปไว้**ในตัวกลุ่ม** (`state.billMergeGroups[].members`) ซึ่งถูกเซฟลง session และโอนข้าม session ผ่าน `mergeBillMergeGroupsInto` อยู่แล้ว (ไม่ต้องแก้จุดนั้น) — ตัด `validationIssues` ออกจากสำเนา (ฟิลด์ยาวสุด และถูกคำนวณใหม่ทุก rebuild อยู่แล้ว)
*   **ปุ่ม "เลิกรวมบิลนี้" ในประวัติการแก้ไข:** ผูกแถวประวัติกับกลุ่มด้วย `mergeGroupId` + `mergeGroupExists()` ตัดสินว่าจะโชว์ปุ่มไหม (เลิกรวมแล้วปุ่มหายเอง) + delegate listener ที่ `auditList`; toast เดิมยังอยู่สำหรับ undo ทันที
*   **กลุ่มเก่าก่อนแพตช์ (ไม่มีสำเนา):** โหมด snapshot = เตือนให้โหลดไฟล์ต้นทางก่อน **และไม่ลบกลุ่มทิ้ง** (ลบแล้วบิลจะหาย); โหมดมีไฟล์ต้นทาง = ทำต่อได้ปกติเพราะ `buildBills()` สร้างใหม่จาก source
*   **ทดสอบ 41 ข้อผ่านหมด:** logic 30 ข้อ (node + ฟังก์ชันจริง, ใช้ `applyManualMergeGroups` ตัวจริงยุบบิล) — จำลองทิ้ง closure ทั้งหมดแล้วเลิกรวมยังได้, แทรกกลับ**ตำแหน่งเดิมกลางลิสต์**ไม่ไปต่อท้าย, กดซ้ำ/id มั่ว = false ไม่พัง, โอนข้าม session สำเนาติดไปด้วย; DOM 11 ข้อ (Chromium จริง) — ปุ่มไม่ทำ grid แถวประวัติเพี้ยน (ยัง 4 คอลัมน์ ลูก 4 ตัว), โชว์เฉพาะกลุ่มที่ยังรวมอยู่, กดแล้วเรียกถูก id
*   **กับดักเทสต์ (จดกันลืม):** วัด `gridTemplateColumns` ของ element ที่ `display:none` ได้ค่าดิบ (`minmax(160px, 0.7fr)` = 2 ชิ้น → นับได้ 8 คอลัมน์) และ `getBoundingClientRect` เป็น 0 ทั้งหมด → **ต้องเปิดแผงให้มองเห็นจริงก่อนวัด** ไม่งั้นข้อที่วัดขนาดจะผ่านแบบไม่มีความหมาย
*   **ยังค้าง:** `bulkMergeGroups` (รวมทีละหลายกลุ่ม — "รวมที่แน่นอน"/"รวมคู่ ORW") **ยังเก็บสำเนาใน closure ของ toast แบบเดิม** ตามขอบเขตที่แพตช์ตั้งใจไม่แตะ — ต้องมี `batchId` + ปุ่ม "เลิกรวมทั้งชุด" เป็นอีกก้อน; วัดขนาด session ต่อการรวม 1 คู่ได้ +173 bytes จากบิลตัวอย่างเล็ก (129 คู่ ≈ +22 KB) **ของจริงสูงกว่านี้เพราะมีรายการยา — ต้องวัดบน live ก่อนทำ bulk merge**
*   deploy + verify กับ live แล้ว (`?v=20260804074158`)

## 📅 3 สิงหาคม 2026 (1) — CKNC: กันเลข AR หลุดเข้าช่อง BAR + บรรทัดนับ AR + เก็บกวาด `[hidden]` 4 จุด

*   **ลงแพตช์ที่วางแผนไว้ที่ร้าน 3 ตัว** (ตาม `PATCH-STATUS-2026-08-03.md`) — เครื่องบ้านตรงกับ live พอดี (`cknc.js` 570,447 bytes) anchor ครบทุกจุด ไม่ต้องเขียนบล็อกใหม่
*   **bar-format-guard (บั๊กข้อมูล):** ทางพิมพ์มือ 4 ที่ (`saveBillOverride` / `applyBulkBarNo` / `applyCardBulkBarNo` / `applyBarPickerSelection`) เดิมรับสตริงอะไรก็ได้ที่ไม่ว่าง → เพิ่ม `normalizeBarInput()` ยึดรูปแบบเดียวกับทาง import (`extractBarNo`); ผิดรูปแบบ = ไม่บันทึก + ช่องแดง + บอกเหตุผล; ว่างยังล้างได้ตามเดิม — เคสจริงที่กัน: `AR-00003-26-3099` หลุดเข้าช่อง BAR 8 ใบ 2 ใบไหลเข้ายอดรายได้
*   **แก้กติกาจากแผนเดิม 1 จุด:** แผนตัดสตริงด้วยจุลภาคทุกชิ้นแล้วบล็อกถ้าชิ้นไหนไม่มี BAR → ทำให้**วางทั้งบรรทัดจากชีตไม่ผ่าน** (เลขเงิน `6,395.00` มีจุลภาคของตัวเอง) ขัดกับเช็กลิสต์ข้อ 5 ของแพตช์เอง; เปลี่ยนเป็นนับชิ้นที่ไม่มี BAR เป็น "ผิด" เฉพาะเมื่อหน้าตาเป็นเลขอ้างอิง (`/[A-Z]{1,5}-\d/`) — ยังบล็อก `AR-...` ครบ
*   **bar-ar-count:** ใต้ช่อง BAR ขึ้น `ใบวางบิลนี้: N เครดิต (AR) · N บิล` อัปเดตสดตอนพิมพ์ กดแล้วเปิด Card Detail มุมมองใหม่ `barBills` (ไม่ผูกตัวกรองวันที่ ให้ตรงกับตัวเลขที่โชว์) — **AR ≠ บิล เมื่อไหร่ = สัญญาณว่าเลขผิด** ได้ตัวตรวจฟรี; รวมกับ guard ข้างบนใน `updateBarEmptyHint()` ตัวเดียว (สองแพตช์เขียนทับฟังก์ชันเดียวกัน)
*   **toast-hidden-fix + เก็บกวาดยกชุด:** `toast.hidden = true` ไม่มีผลเพราะ `.cknc-undo-toast` ตั้ง `display: flex` (กับดัก `[hidden]` ครั้งที่ 3) → เติม `[hidden]` คู่; **ไล่ทั้งไฟล์ด้วยสคริปต์ audit** (17 จุดที่ JS สั่ง `.hidden =` เทียบกับคลาสที่ตั้ง `display`) เจอเพิ่มอีก 3 จุดที่พังเงียบ ๆ อยู่ — `.card-detail-pager` (แถบแบ่งหน้าโผล่ทั้งที่มีหน้าเดียว), `.card-detail-chips`, `.auth-action` — แก้พร้อมกันหมด; ที่เหลือ (`#cknc-supplier-suggest`, `.advanced-badge`) ไม่ได้ตั้ง `display` = ปลอดภัยจริง
*   **ทดสอบ 34 ข้อผ่านหมด:** logic 20 ข้อ — ดึง**ตัวฟังก์ชันจากไฟล์จริง**มารันกับ stub ไม่ก๊อปมาเขียนใหม่ (เคสบล็อก AR, ตัวพิมพ์เล็ก, วางทั้งบรรทัด, ล้างค่า, หลาย BAR, ตัวเลขจาก live 364 = 6/6, เลขปลอม = 2/7); CSS/DOM 14 ข้อ — Chromium จริงเสิร์ฟ `public/` แต่ตัด JS ออก: กด × ปิดได้จริง, หมดเวลาหาย, `เลิกรวม` ทำงานพร้อมปิด, `:empty` ไม่กินที่, 3 จุด `[hidden]` ซ่อนได้แล้วและกลับมาโผล่ได้ตามปกติ
*   **กับดักที่เจอตอนเขียนเทสต์ (จดกันลืม):** ตัวดึงฟังก์ชันด้วยการนับ `{}` ตัดโค้ดผิดเมื่อฟังก์ชันมี destructuring ใน parameter (`showToast(msg, { actionLabel = "" })`) — ต้องข้ามวงเล็บพารามิเตอร์ก่อนค่อยนับ body; และปุ่ม AR2BAR `top` ต่างจากช่อง 5px มาแต่เดิม (ปุ่มสูง 36 ช่อง 27 + `align-items:center`) ยืนยันด้วยการ render `git show HEAD:` เทียบ — ไม่ใช่ regression
*   deploy + verify กับ live แล้ว (`?v=20260803230308`) — ยังเหลือแพตช์ #3–#7, #9 ในคิว และ #10 billed-sweep ที่ยังลงไม่ได้
