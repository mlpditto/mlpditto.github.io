...
*   **Firestore Security Fix (proorder.html):** แก้ไขปัญหา "Invalid document reference" เมื่อใช้ชื่อสินค้าที่มีเครื่องหมาย `/` โดยการเพิ่มระบบ Sanitize ชื่อไฟล์อัตโนมัติก่อนบันทึก `master_products`
*   **TMTP Manager (Tax Invoice System):**
    *   เพิ่มระบบออกใบกำกับภาษี (Tax Invoice) ในรูปแบบ PDF ที่เป็นทางการ
    *   เชื่อมต่อฐานข้อมูล Master Data เพื่อดึงชื่อสินค้าและส่งราคาที่แก้ไขกลับไปบันทึกที่ Master อัตโนมัติ
    *   ระบบจดจำข้อมูลผู้ขาย (Seller Info) และจัดการข้อมูลผู้ซื้อ
*   **อัปเดตเอกสาร (Documentation):** ปรับปรุง `FEATURES.md`, `README.md` และ `TASKS.md` ให้เป็นปัจจุบันที่สุด
